      ...
      <div className="flex w-full max-w-screen-md items-start space-x-4 px-5 sm:px-0">
        <div
          className={clsx(
            ...
            message.role === "assistant" ? "bg-blue-600" : "bg-black"
          )}
        >
        ...
        {message.role === "assistant" && message.isLoading ? (
          <>
            <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden">
              <div className={clsx("h-full bg-black", isFirstMessage ? "animate-spin-slow" : "")} style={{ width: `${progress}%` }}></div>
            </div>
            <div className="w-full flex items-center justify-center text-xs text-red-800">
      ...
      {isFirstMessage && (
        <div className="status-messages">
          <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden mt-8">
            <div className={clsx("h-full bg-black animate-pulse", isFirstMessage ? "animate-spin-slow" : "")} style={{ width: `${progress}%` }}></div>
          </div>
